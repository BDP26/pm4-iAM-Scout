...
class ClubsScraper:
    """Scraper for collecting club and season information from Transfermarkt."""

    def __init__(self, league, start_year=2020, end_year=2026, league_type="amateur"):
        """Initialize clubs scraper with league and season parameters."""
        self.league_url = {
    ...
    def collect_clubs(self):
        """Collect clubs from league pages for all seasons."""
        rows = []

        for season in self.seasons:
            for league in self.league:
                url = self.league_url[league].format(season=season)
        ...
        clubs_dataframe = pd.DataFrame(rows)

        self.clubs_per_season = (
            clubs_dataframe[["club_id", "league", "season"]]
            .drop_duplicates()
            .reset_index(drop=True)
        )

        self.clubs = (
            clubs_dataframe[["club_name", "club_id", "club_slug"]]
    ...
    def collect_locations(self):
        """Collect postal codes and locations for clubs."""
        ...
        for row in self.clubs.itertuples(index=False):
            ...
            try:
                ...
                postal_code, location = self.parser.parse_plz_location(html)
            except Exception as error:
                print(f"[WARN] facts failed for club_id={club_id}, slug={slug}: {error}")

            if not (postal_code and location):
                url_stadium = self.stadium_url.format(slug=slug, club_id=club_id)
                try:
                    html = self.client.get(url_stadium)
                    postal_code, location = self.parser.parse_plz_location_stadium(html)
                except Exception as error:
                    print(f"[WARN] stadium failed for club_id={club_id}, slug={slug}: {error}")

            postal_codes.append(postal_code)
            locations.append(location)

        self.clubs["PLZ"] = postal_codes
        self.clubs["location"] = locations
        self.clubs = self.clubs[["club_id", "club_name", "PLZ", "location", "club_slug"]]

        return self.clubs

    def run(self):
        """Execute complete club scraping workflow."""
        self.collect_clubs()
...
def main(league, start_year, end_year, league_type):
    """Execute club scraper with given parameters."""
